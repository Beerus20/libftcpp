...
const std::string	&Text::getTmpSeparators(void) const
{
	return (this->_tmp_separators);
}

const bool	&Text::getTmpSeparatorStatus(void) const
{
	return (this->_tmp_separator_status);
}
...
void	Text::setTmpSeparatorStatus(const bool &status)
{
	this->_tmp_separator_status = status;
}
...
void	Text::addTmpSeparators(const std::string &sep)
{
	for (std::size_t i(0); i < sep.size(); i++)
	{
		if (this->_tmp_separators.find(sep[i]) == std::string::npos)
			this->_tmp_separators += sep[i];
	}
}

void	Text::rmTmpSeparators(const std::string &sep)
{
	std::size_t	pos(std::string::npos);

	for (std::size_t i(0); i < sep.size(); i++)
	{
		pos = this->_tmp_separators.find(sep[i]);
		if (pos != std::string::npos)
			this->_tmp_separators.erase(pos, 1);
	}
}
